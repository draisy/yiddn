...
$SubCategories ='SELECT * FROM `sub-categories` WHERE `Status`="1" AND  `UseFor`="4" AND `Seo`="'.$backpath3.'"';
$result       = $db->query($SubCategories);
$data2        = $result->fetch_assoc();
$tb_torah_resources = 'SELECT * FROM `tb_torah_resources` WHERE `Status`="1" AND `Seo` LIKE "%'.str_replace("-", "%", $pathInfo).'%" AND (`Category`="'.$data2['Id'].'" OR `Category2`="'.$data2['Id'].'"  OR `Category3`="'.$data2['Id'].'")  ORDER BY `CompanyWebsite` ASC';

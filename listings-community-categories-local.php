...
$backpath4 = urldecode($backpath[4]);
$category = 'SELECT * FROM `categories` WHERE `Status`="1" AND `Seo`="'.$backpath4.'"';
...
<!--Header -->  
 ...
 <!--/Navigation -->
    ...
    <h2><a href="<?=ADDRESS_SITE?>community">Community</a><span> |  
    <a href="<?=ADDRESS_SITE?>listings-community/<?=$data1['CitySeo']?>/<?=$data2['ProvinceSeo']?>"><?=$data1['City']?>, <?=$data2['Province']?></a> | <?=$data3['Title']?></span></h2>
 <br><br>
 
<?php
	$query_categories   = 'SELECT * FROM `categories` WHERE `Status`="1" AND `Seo`="'.$backpath4.'" ORDER BY `Title` ASC';
	$query_category		= $db->query($query_categories);
	$total_results = $query_category->num_rows;
	if($total_results>0){
		while ($rows = $query_category->fetch_assoc()){

			$query1 = 'SELECT * FROM `tb_local` WHERE `Status`="1"   AND `Category1`='.$rows['Id'].' AND (`City`='.$data1['Id'].' OR `City2`='.$data1['Id'].' OR `City3`='.$data1['Id'].')  ORDER BY `CompanyWebsite` ASC';
			...
			while ($row = $result->fetch_assoc()) 
				...
				echo '<article id="maincategory-listings";>
							<ul>
							<li><a class="btn btn-default venoboxframe" title="'.$row['CompanyName'].'" data-type="iframe" href="'.$backpath4.'/'.$row['Seo'].'">'.$row['CompanyWebsite'].'</a></li>
						 </ul>
					</article>';

...
if(isset($_GET['n']) && isset($_GET['c'])) {
 	...
 	$sql  = "UPDATE `tb_local` SET `CompanyLogo`='' Where `OrderId`= '".base64_decode($_GET['i'])."'";
	$res = mysqli_query($db, $sql);
	echo "<script>window.open('update-local-directory-step1?i=".$_GET['i']."','_self')</script>";
} 
?>
<!doctype html>
<html>
...
<title><?=$row_home->Title;?> - Update Local Directory Step 1</title>
...
<br><br>
     ...
     <h3>Local Directory / POST YOUR LISTING</span></h3>
...
?> 
<form enctype="multipart/form-data" class="newform" method="post" action="update-local-directory-step1-script">
...
<a href="images/CompanyLogo/<?php echo $row_result->CompanyLogo;?>" target="_blank"><img src="images/CompanyLogo/<?php echo $row_result->CompanyLogo;?>" width="100" /></a>
     <a href='update-local-directory-step1?i=<?php echo $_GET['i'];?>&n=<?php echo $row_result->CompanyLogo;?>&c=c' onclick='return DeleteLogo();'><!--Delete--></a>
<?php }?> 
<script>
function DeleteLogo()
{
return confirm("Are you sure you want to delete this Logo?");
  ...
  <div class="submit">
  <input type="submit" onClick="return LocalDirectoryStep1()" name="step1"  value="Next >>"/>     
  </div>
   </form>
<?php }?>         
      </fieldset>
...
<script type="text/javascript" src="js/local-directory-step1.js"></script>

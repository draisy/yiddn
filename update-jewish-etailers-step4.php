...
<title><?=$row_home->Title;?> - Update Jewish Retailers Step 4</title>
...
<script type="text/javascript" src="js/jewish-etailers-step4.js"></script>
...
<form enctype="multipart/form-data" class="newform" method="post" action="update-jewish-etailers-step4-script">
<?php
$tb_etailer = "SELECT * FROM `tb_etailer` WHERE `OrderId`='".base64_decode($_GET['i'])."'";
$result = $db->query($tb_etailer);
$total  = $result->num_rows;
if ($total == 0) {
	$tb_services = "SELECT * FROM `tb_services` WHERE `OrderId`='".base64_decode($_GET['i'])."'";
...
<select name="ExpYear" id="ExpYear" title="select a year" style="width:100px;"> 
	 ...
	 echo '<option value="'.$year.'">'.$year.'</option>';
      ...
      <div class="submit">
         <input type="submit"  name="step4" onClick="return JewishEtailersStep4()" value="Submit"/>  
<input type="button" value="<< Previous" onClick="window.location='update-jewish-etailers-step3?i=<?php echo $_GET['i'];?>
';" class="pre"/>      </div>

...
$categories = 'SELECT * FROM `categories` WHERE `Status`="1" AND  `UseFor`="6" AND  `Seo`="'.$backpath2.'"';
$result       = $db->query($categories);
$data1        = $result->fetch_assoc();
$SubCategories ='SELECT * FROM `sub-categories` WHERE `Status`="1" AND  `UseFor`="6" AND `Seo`="'.$backpath3.'"';
$result       = $db->query($SubCategories);
$data2        = $result->fetch_assoc();
$tb_services   = 'SELECT * FROM `tb_services` WHERE `Status`="1" AND `Seo`="'.$pathInfo.'"  AND (`Category`="'.$data2['Id'].'" OR `Category2`="'.$data2['Id'].'"  OR `Category3`="'.$data2['Id'].'") ORDER BY `CompanyWebsite` ASC';
$result       = $db->query($tb_services);
...
<head>
<meta charset="utf-8">
<title><?=$row_home->Title;?> | Services | <?=$data1['Title']?> | <?=$data2['Title']?> | <?=$data3['CompanyName']?></title>
<meta name="keywords" content="<?=$row_home->Title;?>, Services, <?=$data1['Title']?>, <?=$data2['Title']?>, <?=$data3['CompanyName']?>" />
<meta name="Description" content="<?=$row_home->Title;?>,s Services, <?=$data1['Title']?>, <?=$data2['Title']?>, <?=$data3['CompanyName']?>" />
...
<?php if(
 ...
 $data3['Description']!=""
       ...
       <td> 
<br><br><br><br><br><br>
<form method="post" action="<?=ADDRESS_SITE;?>bookmark-action">

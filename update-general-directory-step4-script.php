...
if(isset($_POST['step4'])){
	...
	$sql = "UPDATE `tb_ad_circular` SET `$key` = '$val' WHERE `OrderId` = '".$_POST['orderid']."'";
...
"Summary"		=> isset($_POST['Summary']) ? $_POST['Summary'] : '',
	...
	foreach ($array as $key => $val) {
	$sql = "UPDATE `order` SET `$key` = '$val' WHERE `OrderId` = '".$_POST['orderid']."'";
	$res = mysqli_query($db, $sql);
	}
			$fp = fopen('data.log', "a");
			fwrite($fp, "$refId\r\n");
			...
			fwrite($fp, "$amount\r\n");
			fclose($fp);
$subject  = "Update  Ad Circular ".$row_home->Title;
...
$body .='</td>
  ...
  <tr>
	...
	$body .='
		...
		$subject_ = "Update Listing For ".$row_home->Title;
	...
	$body_ .='
		...
		mail($to_,$subject_,$body_,$headers_);
	        ...
	        header("Location:dashboard");
			 ...
			 header("Location:general-directory-step4?i=".base64_encode($OrderId));	
			}
		}else{

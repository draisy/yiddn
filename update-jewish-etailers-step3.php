...
if(isset($_GET['n']) && isset($_GET['b'])) {
	...
	if ($total == 0) {
	   $sql  = "UPDATE `tb_services` SET `".$_GET['Banner']."`=''  Where `OrderId`= '".$_SESSION['OrderId']."'";
	   $res = mysqli_query($db, $sql);
	}
	else {	
		$sql  = "UPDATE `tb_etailer` SET `".$_GET['Banner']."`=''  Where `OrderId`= '".$_SESSION['OrderId']."'";
		$res = mysqli_query($db, $sql);
   }
	echo "<script>window.open('update-jewish-etailers-step3','_self')</script>";
}
?>
<!doctype html>
<html>
...
<title><?=$row_home->Title;?> - Update Jewish Retailers Step 3</title>
...
<?php 
$query = "select * from `tb_etailer` where `OrderId`= '".base64_decode($_GET['i'])."' AND  `UserId`= '".$_SESSION['Yiddn_login_Id']."'";
$result = $db->query($query);
if($result->num_rows == 0){	
	$query = "select * from `tb_services` where `OrderId`= '".base64_decode($_GET['i'])."' AND  `UserId`= '".$_SESSION['Yiddn_login_Id']."'";
	$result = $db->query($query);
}
 if ($result->num_rows > 0){
...
?>        
<form enctype="multipart/form-data" class="newform" method="post" action="update-jewish-etailers-step3-script">      
    <br>
...
<a href='update-jewish-etailers-step3?n=<?php echo $row_result->Banner;?>&b=b&Banner=Banner' onclick='return DeleteBanner();'>Delete</a>
<?php }?> 
<script>
...
<a href='update-jewish-etailers-step3?n=<?php echo $row_result->Banner2;?>&b=b&Banner=Banner2' onclick='return DeleteBanner();'>Delete</a>
<?php }?> 
<script>
function DeleteBanner()
{
...
<a href='update-jewish-etailers-step3?n=<?php echo $row_result->Banner3;?>&b=b&Banner=Banner3' onclick='return DeleteBanner();'>Delete</a>
<?php }?> 
<script>
function DeleteBanner()
{
...
<input name="OrderId"  value="<?=base64_decode($_GET['i'])?>" id="orderid" type="hidden"  />
    ...
    <div class="submit">
      <input type="submit" onClick="return JewishEtailersStep3()" name="step3"  value="Next >>"/>     
         <input type="button" value="<< Previous" onClick="window.location='update-jewish-etailers-step2?i=<?php echo $_GET['i'];?>
';" class="pre"/>
    </div>
      </form>
    <?php }?>  

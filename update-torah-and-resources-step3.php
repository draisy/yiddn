...
if(isset($_GET['n']) && isset($_GET['b'])) {
   ...
   $sql  = "UPDATE `tb_torah_resources` SET `".$_GET['Banner']."`=''  Where `OrderId`= '".$_SESSION['OrderId']."'";
		$res = mysqli_query($db, $sql);
	echo "<script>window.open('update-torah-and-resources-step3','_self')</script>";
}
?>
<!doctype html>
<html>
...
<title><?=$row_home->Title;?> - Update Torah and Resources Step 3</title>
...
<?php 
$query = "select * from `tb_torah_resources` where `OrderId`= '".base64_decode($_GET['i'])."' AND  `UserId`= '".$_SESSION['Yiddn_login_Id']."'";
...
?>        
<form enctype="multipart/form-data" class="newform" method="post" action="update-torah-and-resources-step3-script">      
    <br>
...
<?php
$query = 'SELECT * FROM `categories` WHERE  `Status`="1" AND `UseFor`="4" ORDER By Title ASC';
$result = $db->query($query);
...
<a href='update-torah-and-resources-step3?n=<?php echo $row_result->Banner;?>&b=b&Banner=Banner' onclick='return DeleteBanner();'>Delete</a>
<?php }?> 
<script>
...
<fieldset class="newfiled">
    <legend><h5>Category 2</h5></legend>
    <div class="inputitem">
...
<a href='update-torah-and-resources-step3?n=<?php echo $row_result->Banner2;?>&b=b&Banner=Banner2' onclick='return DeleteBanner();'>Delete</a>
<?php }?> 
<script>
function DeleteBanner()
{
...
<fieldset class="newfiled">
    <legend><h5>Category 3</h5></legend>
    <div class="inputitem">
      <label class="title"></label><div class="item-cont">
...
<a href='update-torah-and-resources-step3?n=<?php echo $row_result->Banner3;?>&b=b&Banner=Banner3' onclick='return DeleteBanner();'>Delete</a>
<?php }?> 
<script>
function DeleteBanner()
{
...
<input name="OrderId"  value="<?=base64_decode($_GET['i'])?>" id="OrderId" type="hidden"  />
    ...
    <div class="submit">
      <input type="submit" onClick="return TorahResourcesStep3()" name="step3"  value="Next >>"/>     
         <input type="button" value="<< Previous" onClick="window.location='update-torah-and-resources-step2?i=<?php echo $_GET['i'];?>';" class="pre"/>
    </div>
      </form>
    <?php }?>  
      </fieldset>
...
<script src="js/custom.js"></script>
 ...
 <script type="text/javascript" src="js/torah-and-resources-step3.js"></script>

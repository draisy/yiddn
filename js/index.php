...
-->     
 <span><a href="forgot-your-password">Forgot your password?</a></span>                   
  ...
  <?php }else{?>
   <span><a href="<?=ADDRESS_SITE?>dashboard">My Account</a> |  <a href="<?=ADDRESS_SITE?>bookmark">Bookmarks</a> | <a href="logout">Logout  </a> </span>
  <?php }?>
  <a href="https://twitter.com/Yiddn" target="_blank" class="facebook" title="Facebook"></a> 	
  <a href="https://www.facebook.com/pages/Yiddncom-The-Worlds-Leading-Jewish-Web-Directory/711536155532760" target="_blank" class="twitter" title="Twitter"></a>
...
<select id="president" class="flexselect searchfield2" type="text"  placeholder='All Locations'name="president"  tabindex="2" placeholder='City'>
...
if($total > 0){
	while ($row = $result->fetch_assoc()) 
		{
		echo '<option value="'.$row['CitySeo'].'">'.$row['City'].'</option>';	
		}
}
?>
</select>
...
<?php
$count=1;
$city ="SELECT * FROM `city` WHERE Status='1' ORDER BY `City` ASC LIMIT 7";
$result = $db->query($city);
$total  = $result->num_rows;
if($total > 0){
	while ($row = $result->fetch_assoc()) 
...
?>
			...
			<div class="col4">
 				<ul>
					<li><a href="community">See Alls</a></li>
				</ul>
			</div>

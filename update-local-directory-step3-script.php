...
if(isset($_POST['step3'])){
		...
		if($Banner != "")
		{
			...
			$sql = "UPDATE `tb_local` SET `$key` = '$val' WHERE `OrderId` = '".$OrderId."'";
	...
	foreach ($array as $key => $val) {
	$sql = "UPDATE `tb_local` SET `$key` = '$val' WHERE `OrderId` = '".$_SESSION['OrderId']."'";
	$res = mysqli_query($db, $sql);
	}
...
$tb_etailer = 'SELECT * FROM `tb_local` WHERE `OrderId`="'.$_SESSION['OrderId'].'"';
   ...
   <table width="100%" border="0">
     <tr>
	  ...
	  <tr>
        ...
        <td width="57%">';
if($Category1!=""){
$query_subcat = 'SELECT * FROM `categories` where `Status`="1" AND `Id`='.$Category1;
...
$mail->Subject = "New Request Local Directory  - ".$row_home->Title;
$mail->Body=$body;
$mail->send();	
...
$_SESSION['success']="Your ad or listing is currently pending.<br> We will review and confirm the status as soon as possible.";				 		
	        header("Location:dashboard");
	  		 ...
	  		 header("Location:update-local-directory-step4?i=".base64_encode($OrderId));
 
		}
	}else{
		   header("Location:update-local-directory-step3?i=".base64_encode($OrderId));

...
<title><?=$row_home->Title;?> - Update Special Offers Step 4</title>
...
<script type="text/javascript" src="js/general-directory-step4.js"></script>
...
?>               
<form enctype="multipart/form-data" class="newform" method="post" action="update-general-directory-step4-script">
<?php
$tb_ad_circular = 'SELECT * FROM `tb_ad_circular` WHERE `OrderId`="'.$_SESSION['OrderId'].'"';
$result = $db->query($tb_ad_circular);
$total  = $result->num_rows;
$row = $result->fetch_assoc();
?>	       
    <br>
<input type="hidden" id="total" name="total" value="<?=$row['large']+$row['stantard']+$row['small']?>">
<input type="hidden" id="CompanyName" name="CompanyName" value="<?=$row['CompanyName']?>">
<input type="hidden" id="AdTitle" name="AdTitle" value="<?=$row['AdTitle']?>">
<input type="hidden" id="City" name="City" value="<?=$row['City']?>">
<input type="hidden" id="Agent" name="Agent" value="<?=$row['Agent']?>">
<input type="hidden" id="Category" name="Category" value="<?=$row['Category1']?>">
 ...
 <fieldset class="newfiled">
    ...
    <div class="inputitem">
      ...
      <div class="item-cont">
        ...
        <option value="10" <?php if($row['PaymentPlan']==10){echo 'selected="selected"';}?>>Get 10% OFF if paid quarterly</option>
        <option value="20" <?php if($row['PaymentPlan']==20){echo 'selected="selected"';}?>>Get 20% OFF if paid half yearly</option>
        <option value="RECUR" <?php if($row['PaymentPlan']=='RECUR'){echo 'selected="selected"';}?>>Regular Charges as indicated - Charged every month</option>
        <option value="ONETIME" <?php if($row['PaymentPlan']=='ONETIME'){echo 'selected="selected"';}?>>Only charge for one month</option>     
        </select> 
...
<input name="Coupon" id="Coupon" type="text" value="<?=$row['Coupon']?>"  />
...
<input name="NameasonCard" class="large" id="NameasonCard" type="text" value="<?=$rows->NameasonCard?>" />
    </div>
    
    
    <div class="inputitem">
    <label class="title">Card Number:</label>
<input name="CreditCard" class="large" id="CreditCard" type="text" value="<?=$rows->ccnumber?>" />
    </div>
...
<select name="ExpYear" id="ExpYear" title="select a year" style="width:100px;"> 
	 ...
	 echo '<option value="'.$year.'">'.$year.'</option>';
      ...
      <div class="submit">
         <input type="submit"  name="step4" onClick="return GeneralDirectoryStep4()" value="Submit"/>  
         <input type="button" value="<< Previous" onClick="window.location='update-general-directory-step3?i=<?php echo $_GET['i'];?>';" class="pre"/>
      </div>

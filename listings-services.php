...
$SubCategories ='SELECT * FROM `sub-categories` WHERE `Status`="1" AND  `UseFor`="6" AND `Seo`="'.$pathInfo.'"';
...
<head>
<meta charset="utf-8">
<title><?=$row_home->Title;?> | Services | <?=$data1['Title']?> | <?=$data2['Title']?></title>
<meta name="keywords" content="<?=$row_home->Title;?>, Services, <?=$data1['Title']?>, <?=$data2['Title']?>" />
<meta name="Description" content="<?=$row_home->Title;?>,s Services, <?=$data1['Title']?>, <?=$data2['Title']?>" />
...
<!--Header -->  
 ...
 <!--/Navigation -->
    ...
    <h2><a href="<?=ADDRESS_SITE?>services">Services</a><span>  | <a href="../<?=$data1['Seo']?>"><?=$data1['Title']?></a> | <?=$data2['Title']?></span></h2>
...
$count=1;
$a_count=1;
$tb_services = 'SELECT * FROM `tb_services` WHERE `Status`="1" AND (`Category`="'.$data2['Id'].'" OR `Category2`="'.$data2['Id'].'"  OR `Category3`="'.$data2['Id'].'") ORDER BY `CompanyWebsite` ASC';

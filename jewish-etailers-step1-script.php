...
if(isset($_POST['step1'])){
	...
	if(empty($errors)) {
		   ...
		   $sql .= " VALUES ('".implode("', '", $array)."') ";
		   $res = mysqli_query($db, $sql);
		   //duplicate entry for seasonal listings
		   $res = mysqli_query($db, $sql);
		   //new id
		   $update = "UPDATE `order` SET `OrderId`='".$OrderId.'-s'."', `OrderType`='Services'  WHERE `Id`=LAST_INSERT_ID()";
		   $res = mysqli_query($db, $update);		   

		 header("Location:jewish-etailers-step2");
	}else{
		  header("Location:jewish-etailers-step1");
	}
}
?>

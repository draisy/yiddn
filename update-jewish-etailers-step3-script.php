...
if(isset($_POST['step3'])){
		...
		if($Banner != "")
		{
			...
			if ($res->num_rows == 0){
				...
				foreach ($array as $key => $val) {
					$sql = "UPDATE `tb_etailer` SET `$key` = '$val' WHERE `OrderId` = '".$OrderId."'";
					$res = mysqli_query($db, $sql);
				}
			}	
...
$mail->Subject = "New Request Jewish Etailers  - ".$row_home->Title;
$mail->Body=$body;
$mail->send();	
...
$_SESSION['success']="Your ad or listing is currently pending.<br> We will review and confirm the status as soon as possible.";				 		
		   ...
		   header("Location:update-jewish-etailers-step4?i=".base64_encode($OrderId));
		}
			}else{

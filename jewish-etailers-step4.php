...
<title><?=$row_home->Title;?> - Jewish Retailers Step 4</title>
...
<form enctype="multipart/form-data" class="newform" method="post" action="jewish-etailers-step4-script">
<?php
$tb_etailer = 'SELECT * FROM `tb_etailer` WHERE `OrderId`="'.$_SESSION['OrderId'].'"';
$result = $db->query($tb_etailer);
$total  = $result->num_rows;
if ($total == 0) {
	$tb_services = "SELECT * FROM `tb_services` WHERE `OrderId`='".$_SESSION['OrderId'].'-s'."'";
	$result = $db->query($tb_services);
	$total  = $result->num_rows;
//	echo $_SESSION['OrderId'].'-s'.'<br/>';
//	echo $total; print_r($result);
}
...
<select name="ExpYear" id="ExpYear" title="select a year" style="width:100px;"> 
	 ...
	 echo '<option value="'.$year.'">'.$year.'</option>';
      ...
      <div class="submit">
         <input type="submit"  name="step4" onClick="return JewishEtailersStep4s()" value="Submit"/>  
...
if (Return==true){
	window.location='update-jewish-etailers-step3?i=<?=base64_encode($_SESSION['OrderId']);?>';

...
if(isset($_POST['step3'])){
		...
		if($Banner != "")
		{
			...
			$sql = "UPDATE `tb_torah_resources` SET `$key` = '$val' WHERE `OrderId` = '".$OrderId."'";
	...
	foreach ($array as $key => $val) {
$sql = "UPDATE `tb_torah_resources` SET `$key` = '$val' WHERE `OrderId` = '".$_SESSION['OrderId']."'";
	$res = mysqli_query($db, $sql);
	}
...
$mail->Subject = "New Request Torah and Resources  - ".$row_home->Title;
$mail->Body=$body;
$mail->send();	
...
$_SESSION['success']="Your ad or listing is currently pending.<br> We will review and confirm the status as soon as possible.";				 		
			...
			header("Location:update-torah-and-resources-step4?i=".base64_encode($OrderId));
					}	   
	}else{
		   header("Location:update-torah-and-resources-step3?i=".base64_encode($OrderId));
	}
}
?>

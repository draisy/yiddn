...
if(isset($_GET['n']) && isset($_GET['c'])) {
 	...
 	$sql  = "UPDATE `tb_torah_resources` SET `CompanyLogo`='' Where `OrderId`= '".base64_decode($_GET['i'])."'";
	$res = mysqli_query($db, $sql);
	echo "<script>window.open('update-torah-and-resources-step1?i=".$_GET['i']."','_self')</script>";
} 
?>
<!doctype html>
<html>
...
<title><?=$row_home->Title;?> - Update Torah and Resources Step 1</title>
...
<br><br>
     ...
     <h3>Torah and Resources / POST YOUR LISTING</span></h3>
...
?> 
<form enctype="multipart/form-data" class="newform" method="post" action="update-torah-and-resources-step1-script">
  ...
  <textarea id="CompanyAddress" name="CompanyAddress" class="large"  style="height:100px;"><?=$row_result->CompanyAddress;?></textarea>
    ...
    <input name="OrderType"  type="hidden" value="Torah & Resources" />
    </div>
...
<a href="images/CompanyLogo/<?php echo $row_result->CompanyLogo;?>" target="_blank"><img src="images/CompanyLogo/<?php echo $row_result->CompanyLogo;?>" width="100" /></a>
     <a href='update-torah-and-resources-step1?i=<?php echo $_GET['i'];?>&n=<?php echo $row_result->CompanyLogo;?>&c=c' onclick='return DeleteLogo();'><!--Delete--></a>
<?php }?> 
<script>
function DeleteLogo()
{
...
-->    <input type="file" class="file_input" name="CompanyLogo" style="display:none;" />
   ...
   <input name="OrderId"  type="hidden" value="<?=base64_decode($_GET['i'])?>" />

  <div class="submit">
  <input type="submit" onClick="return TorahResourcesStep1()" name="step1"  value="Next >>"/>     
  </div>
   </form>
<?php }?>         
      </fieldset>
...
<script type="text/javascript" src="js/torah-and-resources-step1.js"></script>

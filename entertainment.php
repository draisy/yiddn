...
<title><?=$row_home->Title;?> | Entertainment</title>
<meta name="keywords" content="<?=$row_home->Title;?>, Entertainment" />
<meta name="Description" content="<?=$row_home->Title;?>, Entertainment" />
...
<h2>Entertainment</h2>
   ...
   <p style="font-style:italic;">With dozens of listings of musicians and entertainers — and more being added every week, Yiddn.com is your first stop for Jewish entertainment.</p>
<br>
 <?php
$c = 1;
echo '<div style="float:left; display:inline-block;" class="fifty">'; 
$categories_ = 'SELECT * FROM `categories` WHERE `Status`="1" AND  `UseFor`="5" Order by `Title` ASC';
...
echo '<article id="maincategory-listings"  style="width:100%;">
	   <ul>
		<h2>'.$data['Title'].'</h2>
		';
$SubCategories ='SELECT * FROM `sub-categories` WHERE `Status`="1" AND  `UseFor`="5"  AND `Cid` = "'.$data['Id'].'"  Order by `Title` ASC';
$result = $db->query($SubCategories);
$total  = $result->num_rows;
while ($data_row = $result->fetch_assoc()){
echo '<li><a href="entertainment/'.$data['Seo'].'/'.$data_row['Seo'].'">'.$data_row['Title'].'</a></li>';	
}
echo "</ul></article>"; 

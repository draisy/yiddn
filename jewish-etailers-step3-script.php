...
if(isset($_POST['step3'])){
	...
	if(empty($errors)) {
			...
			/*Check for retailer categories */	
			$category_query = 'SELECT * FROM `sub-categories` WHERE `Id` IN ("'.$Category.'", "'.$Category2.'", "'.$Category3.'") AND `UseFor` = "2"';
			$retailer_result = mysqli_query($db, $category_query);	
			$retailer_array = array( 0 => null, 1 => null, 2 => null);
			$retailer_cat_array = array();
			while ($row = $retailer_result->fetch_assoc()) { 
				$retailer_cat_array[] = $row['Id'];
			}
			//retailer categories should come first for retail listings
			for ($i = 0; $i < count($retailer_cat_array); $i ++){
				$retailer_array[$i] = $retailer_cat_array[$i];
			}
			
	
			$array = array(
			"Category"					=> mysqli_real_escape_string($db,$retailer_array[0]),
			...
			"Category2"					=> mysqli_real_escape_string($db,$retailer_array[1]),
			...
			"Category3" 				=> mysqli_real_escape_string($db,$retailer_array[2]),
	...
	/*Check for services categories selection */	
	$category_query = 'SELECT * FROM `sub-categories` WHERE `Id` IN ("'.$Category.'", "'.$Category2.'", "'.$Category3.'") AND `UseFor` = "6"';
	$service_result = mysqli_query($db, $category_query);
	$cat_array = array();
	
	if ( $service_result->num_rows > 0 ){

		$sql  = "INSERT INTO `tb_services` (`OrderId`, `CompanyName`, `CompanyAddress`, `CompanyEmail`, `CompanyWebsite`, `CompanyLogo`, `ShortDescription`, `Description`, `Agent`, 
		...
		`Coupon`, `Seo`, `DateAdded`, `Status`, `Userid`, `OrderType`, `Telephone`, `step`) SELECT `OrderId`, `CompanyName`, `CompanyAddress`, `CompanyEmail`, `CompanyWebsite`, `CompanyLogo`, `ShortDescription`, `Description`, `Agent`, 
		`CompanyKeywords`, `Category`, `upgrade`, `Premium`, `Banner`, `Category2`, `upgrade2`, `Premium2`, `Banner2`, `Category3`, `upgrade3`, `Premium3`, `Banner3`, `City`, `PaymentPlan`,
		`Coupon`, `Seo`, `DateAdded`, `Status`, `UserId`, `OrderType`, `Telephone`, `step` FROM `tb_etailer` WHERE `OrderId` = '".$_SESSION['OrderId']."'";
		...
		//service categories should come first for service listings
		for ($i = 0; $i < count($cat_array); $i ++){
			if ($i == 0){
				$service_category = "Category";	
			}
			else {
				$service_category = "Category".($i + 1);
			}
			$sql = "UPDATE `tb_services` SET `$service_category` = '$cat_array[$i]' WHERE `OrderId` = '".$_SESSION['OrderId']."'";
			$res = mysqli_query($db, $sql);
		} 
		$sql = "UPDATE `tb_services` SET `OrderId`='".$_SESSION['OrderId'].'-s'."', `OrderType` = 'Services' WHERE `OrderId` = '".$_SESSION['OrderId']."'";
		$res = mysqli_query($db, $sql);
	...
	//delete order entry
		...
		$sql = "DELETE FROM `order` WHERE `OrderId` = '".$_SESSION['OrderId'].'-s'."'";
		$res = mysqli_query($db, $sql);
	}
	/*End If any service categories selected */	
	
		//delete duplicate order entry if no retail categories selected
	if ($retailer_result ->num_rows == 0 ){
		$sql = "DELETE FROM `order` WHERE `OrderId` = '".$_SESSION['OrderId']."'";
		$res = mysqli_query($db, $sql);
		$sql = "DELETE FROM `tb_etailer` WHERE `OrderId` = '".$_SESSION['OrderId']."'";
		$res = mysqli_query($db, $sql);
	}

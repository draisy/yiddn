...
WHERE `tb_etailer`.Status='1' AND `tb_etailer`.Id_Etailer IN (".$record_id_etailer."0) 
...
$query_subcategories = 'SELECT * FROM `categories` WHERE `Status`="1" AND `Id` = "'.$row['Category'].'"  Order by `Title` ASC';
    }
	if($row['Category']!="" && $row['tablename']!="local"){
$query_subcategories = 'SELECT * FROM `sub-categories` WHERE `Status`="1" AND `Id` = "'.$row['Category'].'" Order by `Title` ASC';
    }

...
$categories = 'SELECT * FROM `categories` WHERE `Status`="1" AND  `UseFor`="4" AND  `Seo`="'.$backpath2.'"';
$result     = $db->query($categories);
$data1        = $result->fetch_assoc();
$SubCategories ='SELECT * FROM `sub-categories` WHERE `Status`="1" AND  `UseFor`="4"  AND `Seo`="'.$pathInfo.'"';
...
<head>
<meta charset="utf-8">
<title><?=$row_home->Title;?> | Torah Resources | <?=$data1['Title']?> | <?=$data2['Title']?></title>
<meta name="keywords" content="<?=$row_home->Title;?>, Torah Resources, <?=$data1['Title']?>, <?=$data2['Title']?>" />
<meta name="Description" content="<?=$row_home->Title;?>, Torah Resources, <?=$data1['Title']?>, <?=$data2['Title']?>" />
...
<!--Header -->  
 ...
 <!--/Navigation -->
    ...
    <h2><a href="<?=ADDRESS_SITE?>torah-and-resources">Torah Resources</a><span> | <a href="../<?=$data1['Seo']?>"><?=$data1['Title']?></a> | <?=$data2['Title']?></span></h2>
 <br><br>
	...
	$city_code   = $city->fetch_assoc();			
		$counter = $count++;
				...
				echo '<article id="maincategory-listings" '; if($acount=='9'){echo "style='border:0px;'";$a_count=-2;}echo'>
						<ul>
						<li>
						<a class="btn btn-default venoboxframe" data-type="iframe" title="'.$data_row['CompanyName'].'" href="'.$data2['Seo'].'/'.$data_row['Seo'].'">
...
<?php require_once('ads-listings-torah-and-resources.php'); ?>
...
?>
<?php
$dir = "staticads/torah-and-resources/";
$images = scandir($dir);
$i = rand(2, sizeof($images)-1);
$my_images = array (
  "0" =>  "../../staticads/torah-and-resources/".$images[$i]
);
$filename = randomImage($my_images);
$key = array_search($filename, $my_images); // $key = 2;
$ID_Modal="1";
$dirs = opendir('staticads/torah-and-resources/');
?> 

...
if(isset($_GET['n']) && isset($_GET['b'])) {
   ...
   $sql  = "UPDATE `tb_entertainment` SET `".$_GET['Banner']."`=''  Where `OrderId`= '".$_SESSION['OrderId']."'";
		$res = mysqli_query($db, $sql);
	echo "<script>window.open('update-entertainment-step3','_self')</script>";
}
?>
<!doctype html>
<html>
...
<title><?=$row_home->Title;?> - Update Entertainment Step 3</title>
...
?>        
<form enctype="multipart/form-data" class="newform" method="post" action="update-entertainment-step3-script">      
    <br>
...
<?php
$query = 'SELECT * FROM `categories` WHERE  `Status`="1" AND `UseFor`="5" ORDER By Title ASC';
$result = $db->query($query);
...
<a href='update-entertainment-step3?n=<?php echo $row_result->Banner;?>&b=b&Banner=Banner' onclick='return DeleteBanner();'>Delete</a>
<?php }?> 
<script>
...
<a href='update-entertainment-step3?n=<?php echo $row_result->Banner2;?>&b=b&Banner=Banner2' onclick='return DeleteBanner();'>Delete</a>
<?php }?> 
<script>
function DeleteBanner()
{
...
<a href='update-entertainment-step3?n=<?php echo $row_result->Banner3;?>&b=b&Banner=Banner3' onclick='return DeleteBanner();'>Delete</a>
<?php }?> 
<script>
function DeleteBanner()
{
...
<input name="OrderId"  value="<?=base64_decode($_GET['i'])?>" id="orderid" type="hidden"  />
    ...
    <div class="submit">
      <input type="submit" onClick="return EntertainmentStep3()" name="step3"  value="Next >>"/>     
         <input type="button" value="<< Previous" onClick="window.location='update-entertainment-step2?i=<?php echo $_GET['i'];?>
';" class="pre"/>
    </div>
      </form>
    <?php }?>  
...
<script src="js/custom.js"></script>
 ...
 <script type="text/javascript" src="js/entertainment-step3.js"></script>

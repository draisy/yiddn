...
$headers ='';
$header  ='';
function ShowSeasonalDirectory(){
	$key='';
	global $db;
	$query = 'SELECT * FROM `tb_seasonal` ORDER By `Id_Seasonal`  DESC ';
...
echo $row_account['EmailAddress'];
		...
		echo '</td>
		<td>
  	    <a class="table-actions" data-toggle="modal" href="#myModal-'.$row['Id_Seasonal'].'" title="View"><i class="icon-list-alt"></i></a>
		
		<a class="table-actions" href="seasonal?y='.base64_encode($row['Id_Seasonal']).'&ac='.$row['Status'].'&u='.$row['UserId'].'" title="Status">';
		if($row['Status']=="1"){
   ...
   echo '</a>
   <a class="table-actions" href="edit-seasonal?i='.base64_encode($row['Id_Seasonal']).'" title="Edit">  <i class="icon-pencil"></i></a>
		
 
		<a class="table-actions" href="seasonal?y='.base64_encode($row['Id_Seasonal']).'&oid='.$row['OrderId'].'"  title="Delete" onclick="return confirmDelete();"><i class="icon-trash"></i></a></td>
		</tr>
	
	';
	echo '<div class="modal fade" id="myModal-'.$row['Id_Seasonal'].'">
            <div class="modal-dialog">
              <div class="modal-content">
                <div class="modal-header">
                  <button aria-hidden="true" class="close" data-dismiss="modal" type="button">×</button>
                  <h4 class="modal-title">Seasonal</h4>
                </div>
...
if(isset($_GET['y'])&& !isset($_GET['ac'])){	
		...
		$delete = "delete from `tb_seasonal` where `Id_Seasonal` = $id";
		$run = mysqli_query($db, $delete);
		$delete = "delete from `order` where `OrderId` ='".$_GET['oid']."'";
		$run = mysqli_query($db, $delete);
		if($run==0){
		$errors[] ="Please Try again.";
		@header( "refresh:0;seasonal");
		}else
		{
		$success.= "Record has been successfully deleted.";
		@header("refresh:0;url=seasonal");
		}
}
if(isset($_GET['y'])&& isset($_GET['ac'])){	
		$id = base64_decode($_GET['y']);
		...
		if($action== "1"){
		$insert = "UPDATE `tb_seasonal` SET `Status`='0' where  `Id_Seasonal`='".$id."' ";
...
$subject = "Seasonal  - Reject Your Request - ".$row_home->Title;
  ...
  <tr>
   ...
   <tr>
	...
	$body .='
		...
		echo "<script>window.open('seasonal','_self')</script>";
				}
			}
			}
		}
		elseif($action== "Inactive"){
		$insert = "UPDATE `tb_seasonal` SET `Status`='1' where `Id_Seasonal`='".$id."' ";
		$result = $db->query($insert);
		$row = $run->fetch_assoc();
		// Procedural way
		$run = mysqli_query($db, $insert);
...
$info 	  = "SELECT * FROM `tb_seasonal` where `Id_Seasonal`= '".$id."'";
$run_info = mysqli_query($db, $info);
$row_info = $run_info->fetch_assoc();

$Email   = $row['EmailAddress']; 
$subject = "Your listing has been approved - Seasonal - ".$row_home->Title;
$from    = $row_home->EmailFrom;
$to      = $Email;
$ip      = isset($_SERVER['REMOTE_ADDR']) ? $_SERVER['REMOTE_ADDR'] : '';
$parts = explode('@', $Email);

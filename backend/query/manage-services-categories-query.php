...
$errors = array();
$success = null;
function ShowServicesCategories(){
	$key='';
	global $db;
	$query = 'SELECT * FROM `categories` WHERE `UseFor`="6" ORDER By Id DESC';
	...
	echo '
		...
		<a class="table-actions" href="manage-services-categories?y='.base64_encode($row['Id']).'&ac='.$row['Status'].'" title="Status">';
		if($row['Status']=="1"){
   ...
   echo '</a>
		<a class="table-actions" href="edit-services-category?y='.base64_encode($row['Id']).'" title="Edit"><i class="icon-pencil"></i></a>
		<a class="table-actions" href="manage-services-categories?y='.base64_encode($row['Id']).'" title="Delete" onclick="return confirmDelete();"><i class="icon-trash"></i></a></td>
		</tr>
	';
            ...
            <div class="modal-dialog">
              <div class="modal-content">
                <div class="modal-header">
                  ...
                  <h4 class="modal-title">Services Category</h4>
                </div>
...
if(isset($_GET['y'])&& !isset($_GET['ac'])){	
		...
		@header( "refresh:0;manage-services-categories");
		}else
		{
...
if(isset($_GET['y'])&& isset($_GET['ac'])){	
		...
		if($action== "1"){
		$insert = "UPDATE `categories` SET `Status`='0' where `UseFor`='6' AND  `Id`='".$id."' ";
		...
		$insert = "UPDATE `categories` SET `Status`='1' where `UseFor`='6' AND  `Id`='".$id."' ";
		$result = $db->query($insert);
		// Procedural way
		$run = mysqli_query($db, $insert);

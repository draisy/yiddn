...
$errors = array();
$success = null;
function ShowBusinessDirectoryCategories(){
	$key='';
	global $db;
	$query = 'SELECT * FROM `categories` WHERE `UseFor`="2" ORDER By Id DESC';
	...
	echo '
		...
		<a class="table-actions" href="manage-jewish-e-tailers-categories?y='.base64_encode($row['Id']).'&ac='.$row['Status'].'" title="Status">';
		if($row['Status']=="1"){
   ...
   echo '</a>
		<a class="table-actions" href="edit-jewish-e-tailers-category?y='.base64_encode($row['Id']).'" title="Edit"><i class="icon-pencil"></i></a>
		<a class="table-actions" href="manage-jewish-e-tailers-categories?y='.base64_encode($row['Id']).'" title="Delete" onclick="return confirmDelete();"><i class="icon-trash"></i></a>
<a class="table-actions" href="manage-jewish-e-tailers-categories?y='.base64_encode($row['Id']).'&mf='; 
if($row['Mf']=='yes'){
echo 'no';
}elseif($row['Mf']=='no'){
echo 'yes';
...
if(isset($_GET['y'])&& isset($_GET['mf'])){	
		...
		$action = $_GET['mf'];
		if($action == "yes"){
	$UPDATE = "UPDATE `categories` SET `Mf`='yes' where `UseFor`='2' AND  `Id`='".$id."' ";
	$query = $db->query($UPDATE);
		$_SESSION["success"] = "Disable Make Feature Successfully.";	
	@header("refresh:0;url=manage-jewish-e-tailers-categories");
		}
		elseif($action == "no"){
	$UPDATE = "UPDATE `categories` SET `Mf`='no' where `UseFor`='2' AND  `Id`='".$id."' ";
	$query = $db->query($UPDATE);
...
if(isset($_GET['y'])&& !isset($_GET['ac']) && !isset($_GET['mf'])){	
		...
		@header( "refresh:0;manage-jewish-e-tailers-categories");
		}else
		{
...
if(isset($_GET['y'])&& isset($_GET['ac'])){	
		...
		if($action== "1"){
		$insert = "UPDATE `categories` SET `Status`='0' where `UseFor`='2' AND  `Id`='".$id."' ";
		...
		$insert = "UPDATE `categories` SET `Status`='1' where `UseFor`='2' AND  `Id`='".$id."' ";
		$result = $db->query($insert);
		// Procedural way
		$run = mysqli_query($db, $insert);

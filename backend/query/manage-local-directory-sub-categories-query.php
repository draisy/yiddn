...
$errors = array();
$success = null;
function ShowLocalDirectorySubCategories(){
	$key='';
	global $db;
	$query = 'SELECT * FROM `sub-categories` WHERE `UseFor`="3" ORDER By Id DESC';
	...
	echo '
		...
		<a class="table-actions" href="manage-local-directory-sub-categories?y='.base64_encode($row['Id']).'&ac='.$row['Status'].'" title="Status">';
		if($row['Status']=="1"){
   ...
   echo '</a>
		<a class="table-actions" href="edit-local-directory-sub-category?y='.base64_encode($row['Id']).'" title="Edit"><i class="icon-pencil"></i></a>
		<a class="table-actions" href="manage-local-directory-sub-categories?y='.base64_encode($row['Id']).'" title="Delete" onclick="return confirmDelete();"><i class="icon-trash"></i></a></td>
		</tr>
	';
...
if(isset($_GET['y'])&& !isset($_GET['ac'])){	
		...
		@header( "refresh:0;manage-local-directory-sub-categories");
		}else
		{

...
$headers ='';
$header  ='';
function ShowLocalDirectory(){
	$key='';
	global $db;
	$query = 'SELECT * FROM `tb_local` ORDER By `Id_Local` DESC';
...
echo $row_account['EmailAddress'];
		...
		echo '</td>
		<td>
  	    <a class="table-actions" data-toggle="modal" href="#myModal-'.$row['Id_Local'].'" title="View"><i class="icon-list-alt"></i></a>
		<a class="table-actions" href="local-directory?y='.base64_encode($row['Id_Local']).'&ac='.$row['Status'].'&u='.$row['UserId'].'" title="Status">';
		if($row['Status']=="1"){
   ...
   echo '</a>
     	 <a class="table-actions" href="edit-local-directory?i='.base64_encode($row['Id_Local']).'" title="Edit">  <i class="icon-pencil"></i></a>
		 <a class="table-actions" href="local-directory?y='.base64_encode($row['Id_Local']).'&oid='.$row['OrderId'].'" title="Delete" onclick="return confirmDelete();"><i class="icon-trash"></i></a></td>
		</tr>
	
	';
	echo '<div class="modal fade" id="myModal-'.$row['Id_Local'].'">
            <div class="modal-dialog">
              <div class="modal-content">
                <div class="modal-header">
                  <button aria-hidden="true" class="close" data-dismiss="modal" type="button">×</button>
                  <h4 class="modal-title">Local Directory</h4>
                </div>
...
if($row['City2']!=""){
$query_city = 'SELECT * FROM `city` where `Status`="1" AND `Id`='.$row['City2'];
$result_city = $db->query($query_city);
$row_city= $result_city->fetch_assoc(); 
echo $row_city['City'];
}
...
if($row['City3']!=""){
$query_city = 'SELECT * FROM `city` where `Status`="1" AND `Id`='.$row['City3'];
$result_city = $db->query($query_city);
$row_city= $result_city->fetch_assoc(); 
echo $row_city['City'];
}
...
if($row['Category1']!=""){
$query_category = 'SELECT * FROM `categories` where `Status`="1" AND `Id`='.$row['Category1'];
...
if(isset($_GET['y'])&& !isset($_GET['ac'])){	
		...
		$delete = "delete from `tb_local` where `Id_Local` = $id";
		$run = mysqli_query($db, $delete);
		$delete = "delete from `order` where `OrderId` ='".$_GET['oid']."'";
		$run = mysqli_query($db, $delete);
		if($run==0){
		$errors[] ="Please Try again.";
		@header( "refresh:0;local-directory");
		}else
		{
		$success.= "Record has been successfully deleted.";
		@header("refresh:0;url=local-directory");
		}
}
if(isset($_GET['y'])&& isset($_GET['ac'])){	
		$id = base64_decode($_GET['y']);
		...
		if($action== "1"){
		$UPDATE = "UPDATE `tb_local` SET `Status`='0' where  `Id_Local`='".$id."' ";
...
$mail->Subject = "Local Directory  - Reject Your Request - ".$row_home->Title;
$mail->Body=$body;
		if($mail->send()){
			///End Email
		...
		echo "<script>window.open('local-directory','_self')</script>";
				}
			}
		}
		elseif($action== "Inactive"){
		$insert = "UPDATE `tb_local` SET `Status`='1' where `Id_Local`='".$id."' ";
...
$info 	  = "SELECT * FROM `tb_local` where `Id_Local`= '".$id."'";
...
$body .='</td>
	  ...
	  <tr>
        <td valign="top"><strong>Upgrade to premium for maximum impact with a banner ad display - for only $199.00 a month.	</strong></td>
       <td>';
		if($row_info['Premium']!=""){
			$body .= "Yes";
			}else{
	...
	if($row_info['Banner']!=""){
	 ...
	 $body .= '</td>
	   ...
	   <tr>
        <td valign="top"><strong>City 2</strong></td>
        <td>';
if($row_info['City2']!=""){
$query_city = 'SELECT * FROM `city` where `Status`="1" AND `Id`='.$row_info['City2'];
$result_city = $db->query($query_city);
$row_city= $result_city->fetch_assoc(); 
$body .= $row_city['City'];
}
$body .='</td>
	...
	if($row_info['Banner2']!=""){
	 ...
	 $body .= '</td>
	  ...
	  <tr>
        <td valign="top"><strong>City 3</strong></td>
      <td>';
if($row_info['City3']!=""){
$query_city = 'SELECT * FROM `city` where `Status`="1" AND `Id`='.$row_info['City3'];
$result_city = $db->query($query_city);
$row_city= $result_city->fetch_assoc(); 
$body .= $row_city['City'];
}
	...
	$body .= '<img src="http://www.yiddn.com/images/Banner/'.$row_info['Banner3'].'" style="max-width:200px;">';
	 ...
	 $body .='</td>
	   ...
	   <tr>
        ...
        <td>';
	$category 	  = "SELECT * FROM `categories` where `Id`='".$row_info['Category1']."'";
	$run_category = mysqli_query($db, $category);
	$row_category = $run_category->fetch_assoc();
	$body 		 .= $row_category['Title'];
	$body        .= '</td>
      </tr>
	  <tr>
...
$mail->addReplyTo($row_home->EmailTo, $row_home->Title);
$mail->addAddress($Email, $row_home->Title); 
$mail->Subject = "Your listing has been approved - Local Directory - ".$row_home->Title;
$mail->Body=$body;
		if($mail->send()){
			///End Email
...
{
return confirm("Are you sure you want to delete this Local Directory?");
}
</script>
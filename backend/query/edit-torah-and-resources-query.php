...
$DateAdded 			= date("Y-m-d H:i:s");	
	   ...
	   if(!empty($_POST['CompanyWebsite'])) {
		...
		if($CompanyLogo != "")
		{
			...
			$sql  = "UPDATE `tb_torah_resources` SET `CompanyLogo` = '$UploadCompanyLogo' WHERE `Id_torah_resources` = '$Id'";	
		...
		if($Banner != "")
		{
			...
			$sql  = "UPDATE `tb_torah_resources` SET `Banner` = '$UploadBanner' WHERE `Id_torah_resources` = '$Id'";	
			$res = mysqli_query($db, $sql);
			}else{
				$errors[] ="* Invalid file type. Only JPG, GIF and PNG types are accepted.";
				}
		...
		if($Banner2 != "")
		{
			...
			$sql  = "UPDATE `tb_torah_resources` SET `Banner2` = '$UploadBanner2' WHERE `Id_torah_resources` = '$Id'";	
			$res = mysqli_query($db, $sql);
			}else{
				$errors[] ="* Invalid file type. Only JPG, GIF and PNG types are accepted.";
				}
	...
	/*---Category 3----*/
		...
		if($Banner3 != "")
		{
			...
			$sql  = "UPDATE `tb_torah_resources` SET `Banner3` = '$UploadBanner3' WHERE `Id_torah_resources` = '$Id'";	
			$res = mysqli_query($db, $sql);
			}else{
				$errors[] ="* Invalid file type. Only JPG, GIF and PNG types are accepted.";
				}
...
//			"OrderId"					=> $OrderId,
		...
		$sql = "UPDATE `tb_torah_resources` SET `$key` = '$val' WHERE `Id_torah_resources` = '$Id'";
		$res = mysqli_query($db, $sql);
		}
		$success = "Update Successfully.";	
	      echo '<meta http-equiv="refresh" content="2; url=torah-and-resources" />';
		}
 }
?>
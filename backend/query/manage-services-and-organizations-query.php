...
$headers ='';
$header  ='';
function ShowServicesOrganizations(){
	$key='';
	global $db;
	$query = 'SELECT * FROM `tb_torah_resources` ORDER By `Id_torah_resources` DESC';
...
echo $row_account['EmailAddress'];
		...
		echo '</td>
		<td>
  	    <a class="table-actions" data-toggle="modal" href="#myModal-'.$row['Id_torah_resources'].'" title="View"><i class="icon-list-alt"></i></a>
		<a class="table-actions" href="torah-and-resources?y='.base64_encode($row['Id_torah_resources']).'&ac='.$row['Status'].'&u='.$row['UserId'].'" title="Status">';
		if($row['Status']=="1"){
   ...
   echo '</a>
      ...
      <a class="table-actions" href="edit-torah-and-resources?i='.base64_encode($row['Id_torah_resources']).'" title="Edit">  <i class="icon-pencil"></i></a>

		<a class="table-actions" href="torah-and-resources?y='.base64_encode($row['Id_torah_resources']).'&oid='.$row['OrderId'].'" title="Delete" onclick="return confirmDelete();"><i class="icon-trash"></i></a></td>
		</tr>
	
	';
	echo '<div class="modal fade" id="myModal-'.$row['Id_torah_resources'].'">
            <div class="modal-dialog">
              <div class="modal-content">
                <div class="modal-header">
                  <button aria-hidden="true" class="close" data-dismiss="modal" type="button">×</button>
                  <h4 class="modal-title">Torah & Resources</h4>
                </div>
...
if(mysqli_real_escape_string($db,$row['Category'])!=""){
...
if(mysqli_real_escape_string($db,$row['Category2'])!=""){
...
$query_category = 'SELECT * FROM `sub-categories` where `Status`="1" AND `Id`='.$row['Category3'];
$result_category = $db->query($query_category);
$row_category = $result_category->fetch_assoc(); 
echo $row_category['Title'];
}
...
$delete = "delete from `tb_torah_resources` where `Id_torah_resources` = $id";
		$run = mysqli_query($db, $delete);
		$delete = "delete from `order` where `OrderId` ='".$_GET['oid']."'";
		$run = mysqli_query($db, $delete);
		if($run==0){
		$errors[] ="Please Try again.";
		@header( "refresh:0;torah-and-resources");
		}else
		{
		$success.= "Record has been successfully deleted.";
		@header("refresh:0;url=torah-and-resources");
		}
}
if(isset($_GET['y'])&& isset($_GET['ac'])){	
		$id = base64_decode($_GET['y']);
		...
		if($action== "1"){
		$UPDATE = "UPDATE `tb_torah_resources` SET `Status`='0' where  `Id_torah_resources`='".$id."' ";
...
$mail->Subject = "Torah & Resources  - Reject Your Request - ".$row_home->Title;
$mail->Body=$body;
		if($mail->send()){
			///End Email
		...
		echo "<script>window.open('torah-and-resources','_self')</script>";
				}
			}
		}
		elseif($action== "Inactive"){
		$insert = "UPDATE `tb_torah_resources` SET `Status`='1' where `Id_torah_resources`='".$id."' ";
...
$run = mysqli_query($db, $select);
$row = $run->fetch_assoc();
$info 	  = "SELECT * FROM `tb_torah_resources` where `Id_torah_resources`= '".$id."'";
...
$body .='<div dir="ltr" style="width:600px;"><table width="600" border="0" align="left" cellpadding="0" cellspacing="0">
  ...
  <tr>
    ...
    <tr>    <td><strong>Dear '.$row['FirstName'].',</strong></td>
</tr>
...
$body .='</td>
	...
	if($row_info['Banner2']!=""){
	 ...
	 $body .= '</td>
      ...
      <td>';
if($row_info['Category3']!=""){
$query_subcat = 'SELECT * FROM `sub-categories` where `Status`="1" AND `Id`='.$row_info['Category3'];
$result_subcat = $db->query($query_subcat);
$row_subcat = $result_subcat->fetch_assoc(); 
$body .= $row_subcat['Title'];
}
...
$mail->addReplyTo($row_home->EmailTo, $row_home->Title);
$mail->addAddress($Email, $row_home->Title); 
$mail->Subject = "Your listing has been approved - Torah & Resources - ".$row_home->Title;
$mail->Body=$body;
		if($mail->send()){
			///End Email
...
{
return confirm("Are you sure you want to delete this Torah & Resources?");
}
</script> 
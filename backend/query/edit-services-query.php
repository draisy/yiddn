...
$DateAdded 			= date("Y-m-d H:i:s");	
	   ...
	   if(!empty($_POST['CompanyWebsite'])) {
		...
		if($CompanyLogo != "")
		{
			...
			$sql  = "UPDATE `tb_services` SET `CompanyLogo` = '$UploadCompanyLogo' WHERE `Id_Services` = '$Id'";	
		...
		if($Banner != "")
		{
			...
			$sql  = "UPDATE `tb_services` SET `Banner` = '$UploadBanner' WHERE `Id_Services` = '$Id'";	
			$res = mysqli_query($db, $sql);
			}else{
				$errors[] ="* Invalid file type. Only JPG, GIF and PNG types are accepted.";
				}
		...
		if($Banner2 != "")
		{
			...
			$sql  = "UPDATE `tb_services` SET `Banner2` = '$UploadBanner2' WHERE `Id_Services` = '$Id'";	
			$res = mysqli_query($db, $sql);
			}else{
				$errors[] ="* Invalid file type. Only JPG, GIF and PNG types are accepted.";
				}
	...
	/*---Category 3----*/
		...
		if($Banner3 != "")
		{
			...
			$sql  = "UPDATE `tb_services` SET `Banner3` = '$UploadBanner3' WHERE `Id_Services` = '$Id'";	
			$res = mysqli_query($db, $sql);
			}else{
				$errors[] ="* Invalid file type. Only JPG, GIF and PNG types are accepted.";
				}
...
//			"OrderId"					=> $OrderId,
		...
		$sql = "UPDATE `tb_services` SET `$key` = '$val' WHERE `Id_Services` = '$Id'";
		$res = mysqli_query($db, $sql);
		}
		$success = "Update Successfully.";	
	      echo '<meta http-equiv="refresh" content="2; url=services" />';
		}
 }
?>
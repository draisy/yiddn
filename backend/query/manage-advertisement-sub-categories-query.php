...
$errors = array();
$success = null;
function ShowAdvertisementSubCategories(){
	$key='';
	global $db;
	$query = 'SELECT * FROM `sub-categories` WHERE `UseFor`="1" ORDER By Id DESC';
	...
	echo '
		...
		<a class="table-actions" href="manage-adcircular-sub-categories?y='.base64_encode($row['Id']).'&ac='.$row['Status'].'" title="Status">';
		if($row['Status']=="1"){
   ...
   echo '</a>
		<a class="table-actions" href="edit-adcircular-sub-category?y='.base64_encode($row['Id']).'" title="Edit"><i class="icon-pencil"></i></a>
		<a class="table-actions" href="manage-adcircular-sub-categories?y='.base64_encode($row['Id']).'" title="Delete" onclick="return confirmDelete();"><i class="icon-trash"></i></a></td>
		</tr>
            ...
            <div class="modal-dialog">
              <div class="modal-content">
                <div class="modal-header">
                  ...
                  <h4 class="modal-title">General Directory Sub Category</h4>
                </div>
...
if(isset($_GET['y'])&& !isset($_GET['ac'])){	
		...
		@header( "refresh:0;manage-adcircular-sub-categories");
		}else
		{
		$success.= "Record has been successfully deleted.";
		@header("refresh:0;url=manage-adcircular-sub-categories");
		}
}
if(isset($_GET['y'])&& isset($_GET['ac'])){	
		$id = base64_decode($_GET['y']);

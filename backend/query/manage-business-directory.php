...
function ShowBusinessDirectory(){
	$key='';
	global $db;
	$query = 'SELECT * FROM `tb_etailer` ORDER By `Id_Etailer`  DESC ';
...
echo $row_account['EmailAddress'];
		...
		echo '</td>
		<td>
  	    <a class="table-actions" data-toggle="modal" href="#myModal-'.$row['Id_Etailer'].'" title="View"><i class="icon-list-alt"></i></a>
		
		<a class="table-actions" href="jewish-e-tailers?y='.base64_encode($row['Id_Etailer']).'&ac='.$row['Status'].'&u='.$row['UserId'].'" title="Status">';
		if($row['Status']=="1"){
   ...
   echo '</a>
   <a class="table-actions" href="edit-jewish-e-tailers?i='.base64_encode($row['Id_Etailer']).'" title="Edit">  <i class="icon-pencil"></i></a>
		
 
		<a class="table-actions" href="jewish-e-tailers?y='.base64_encode($row['Id_Etailer']).'&oid='.$row['OrderId'].'"  title="Delete" onclick="return confirmDelete();"><i class="icon-trash"></i></a></td>
		</tr>
	
	';
	echo '<div class="modal fade" id="myModal-'.$row['Id_Etailer'].'">
            <div class="modal-dialog">
              <div class="modal-content">
                <div class="modal-header">
                  <button aria-hidden="true" class="close" data-dismiss="modal" type="button">×</button>
                  <h4 class="modal-title">Jewish E-Tailers</h4>
                </div>
...
if(isset($_GET['y'])&& !isset($_GET['ac'])){	
		...
		$delete = "delete from `tb_etailer` where `Id_Etailer` = $id";
		$run = mysqli_query($db, $delete);
		$delete = "delete from `order` where `OrderId` ='".$_GET['oid']."'";
		$run = mysqli_query($db, $delete);
		if($run==0){
		$errors[] ="Please Try again.";
		@header( "refresh:0;jewish-e-tailers");
		}else
		{
		$success.= "Record has been successfully deleted.";
		@header("refresh:0;url=jewish-e-tailers");
		}
}
if(isset($_GET['y'])&& isset($_GET['ac'])){	
		$id = base64_decode($_GET['y']);
		...
		if($action== "1"){
		$insert = "UPDATE `tb_etailer` SET `Status`='0' where  `Id_Etailer`='".$id."' ";
...
$mail->Subject = "Jewish E-Tailers  - Reject Your Request - ".$row_home->Title;
$mail->Body=$body;
		if($mail->send()){
			///End Email
		...
		echo "<script>window.open('jewish-e-tailers','_self')</script>";
				}
			}
		}
		elseif($action== "Inactive"){
		$insert = "UPDATE `tb_etailer` SET `Status`='1' where `Id_Etailer`='".$id."' ";
		$result = $db->query($insert);
		$row = $run->fetch_assoc();
		// Procedural way
		$run = mysqli_query($db, $insert);
...
$run = mysqli_query($db, $select);
$row = $run->fetch_assoc();
$info 	  = "SELECT * FROM `tb_etailer` where `Id_Etailer`= '".$id."'";
...
$mail->addReplyTo($row_home->EmailTo, $row_home->Title);
$mail->addAddress($Email, $row_home->Title); 
$mail->Subject = "Your listing has been approved - Jewish E-Tailers - ".$row_home->Title;
$mail->Body=$body;
		if($mail->send()){
			///End Email
...
{
return confirm("Are you sure you want to delete this Jewish E-Tailers?");
}
</script> 
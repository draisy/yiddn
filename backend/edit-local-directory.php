...
if(isset($_GET['n']) && isset($_GET['c'])) {
 	...
 	$sql  = "UPDATE `tb_local` SET `CompanyLogo`='' Where `Id_Local`= '".base64_decode($_GET['i'])."'";
	$res = mysqli_query($db, $sql);
	echo "<script>window.open('edit-local-directory?i=".$_GET['i']."','_self')</script>";
}
...
if(isset($_GET['n']) && isset($_GET['b'])) {
   ...
   $sql  = "UPDATE `tb_local` SET `".$_GET['Banner']."`=''  Where `Id_Local`= '".base64_decode($_GET['i'])."'";
		$res = mysqli_query($db, $sql);
	echo "<script>window.open('edit-local-directory?i=".$_GET['i']."','_self')</script>";
}
...
?>
<!DOCTYPE html>
<html><?php require_once('query/edit-local-directory-query.php'); ?>
<head>
<title>Edit Local Directory - <?php echo $row_home->Title;?></title>
...
<script src="../js/local-validationFun.js"></script>
<script>
      ...
      <div class="row">
        <div class="col-lg-12">
          <div class="widget-container fluid-height clearfix">
            <div class="heading"><i class="icon-edit-sign"></i>Edit Local Directory</div>
...
<?php 
$query = "select * from `tb_local` where `Id_Local`= '".base64_decode($_GET['i'])."'";
...
?> 
       ...
       <fieldset>
        ...
        <input name="CompanyName" id="CompanyName"  class="form-control" type="text" value="<?php echo $row_result->CompanyName;?>" />
        </div>
         <div class="form-group">
        <input type="hidden" id="Id" name="Id" value="<?php echo $row_result->Id_Local;?>" />
        <label for="Address">Company Address:</label>
        <textarea name="CompanyAddress"  class="widget-content" id="CompanyAddress" style="height:40px;"><?php echo $row_result->CompanyAddress;?></textarea>
          </div>
<div class="form-group">
...
<input name="Telephone" id="Telephone" class="form-control"  type="text" value="<?php echo $row_result->Telephone;?>" />
        ...
        <input name="CompanyEmail" class="form-control" id="CompanyEmail" value="<?php echo $row_result->CompanyEmail;?>" type="text" />
        </div>
        
         <div class="form-group">
        
        <label for="CompanyWebsite">Company Website:</label>
        <input name="CompanyWebsite" class="form-control" id="CompanyWebsite" type="text" value="<?php echo $row_result->CompanyWebsite;?>" placeholder="http://" />
        </div>
...
<a href="../images/CompanyLogo/<?php echo $row_result->CompanyLogo;?>" target="_blank"><img src="../images/CompanyLogo/<?php echo $row_result->CompanyLogo;?>" width="100" /></a>
     <a href='update-local?i=<?php echo $_GET['i'];?>&n=<?php echo $row_result->CompanyLogo;?>&c=c' onclick='return DeleteLogo();'>Delete</a>
<?php }?> 
<script>
function DeleteLogo()
{
return confirm("Are you sure you want to delete this Logo?");
        ...
        <textarea id="ShortDescription"   class="widget-content" name="ShortDescription" style="height:40px;"><?php echo $row_result->ShortDescription;?></textarea>
        </div>
        ...
        <input name="CompanyKeywords" class="form-control"  id="CompanyKeywords" type="text" value="<?php echo $row_result->CompanyKeywords;?>" />
    	</div>
...
<select name="City" id="City" class="form-control">
  ...
  <div class="form-group">
        <input type="checkbox" disabled="disabled"  name="Premium" value="199" id="Premium"  class="chk" onclick="showMe('div1')" <?php if($row_result->Premium=='199'){echo 'checked="checked"';}?> />
        <label for="Premium">Upgrade to premium for maximum impact with a banner ad display - for only $199.00 a month.</label>
  </div>
...
<a href='edit-local-directory?i=<?php echo $_GET['i'];?>&n=<?php echo $row_result->Banner;?>&b=b&Banner=Banner' onclick='return DeleteBanner();'>Delete</a>
<?php }?> 
<script>
...
<div class="form-group">
          <input type="checkbox" disabled="disabled"  name="Premium2" value="199" id="Premium2" class="chk" onclick="showMe('div2')" <?php if($row_result->Premium2=='199'){echo 'checked="checked"';}?> />
          <label for="Premium2">Upgrade to premium for maximum impact with a banner ad display - for only $199.00 a month.</label>
</div>
...
<img src="../images/Banner/<?php echo $row_result->Banner2;?>" width="100" /></a>
    <a href='edit-local-directory?i=<?php echo $_GET['i'];?>&n=<?php echo $row_result->Banner2;?>&b=b&Banner=Banner2' onclick='return DeleteBanner();'>Delete</a>
    <?php }?> 
    <script>
		function DeleteBanner()
		{
...
<div class="form-group">
          <input type="checkbox" disabled="disabled"  name="Premium3" value="199" id="Premium3" class="chk" onclick="showMe('div3')" <?php if($row_result->Premium3=='199'){echo 'checked="checked"';}?>/>
          <label for="Premium3">Upgrade to premium for maximum impact with a banner ad display - for only $199.00 a month.</label>
</div>
...
<img src="../images/Banner/<?php echo $row_result->Banner3;?>" width="100" /></a>
        <a href='edit-local-directory?i=<?php echo $_GET['i'];?>&n=<?php echo $row_result->Banner3;?>&b=b&Banner=Banner3' onclick='return DeleteBanner();'>Delete</a>
        <?php }?> 
        <script>
            function DeleteBanner()
            {
...
<div class="form-group">
          ...
          <br /><small>Developer note: This section will only have categories and NO sub-categories</small>
</div>
        </fieldset>
...
<input type="hidden" name="OrderType"      id="OrderType" value="Local Directory" />
      </fieldset>
      </span>

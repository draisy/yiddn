...
if(isset($_GET['n']) && isset($_GET['c'])) {
 	...
 	$sql  = "UPDATE `tb_services` SET `CompanyLogo`='' Where `Id_Services`= '".base64_decode($_GET['i'])."'";
	$res = mysqli_query($db, $sql);
	echo "<script>window.open('edit-services?i=".$_GET['i']."','_self')</script>";
}
...
if(isset($_GET['n']) && isset($_GET['b'])) {
		...
		{  
  		...
  		$sql  = "UPDATE `tb_services` SET `".$_GET['Banner']."`=''  Where `Id_Services`= '".base64_decode($_GET['i'])."'";
		$res = mysqli_query($db, $sql);
	echo "<script>window.open('edit-services?i=".$_GET['i']."','_self')</script>";
}
?>
<!DOCTYPE html>
<html><?php require_once('query/edit-services-query.php'); ?>
<head>
      ...
      <div class="row">
        <div class="col-lg-12">
          <div class="widget-container fluid-height clearfix">
            <div class="heading"><i class="icon-edit-sign"></i>Edit Services</div>
...
<?php 
$query = "select * from `tb_services` where `Id_Services`= '".base64_decode($_GET['i'])."'";
...
?> 
      ...
      <fieldset>
        ...
        <input type="hidden" id="Id" name="Id" value="<?php echo $row_result->Id_Services;?>" />
...
<a href="../images/CompanyLogo/<?php echo $row_result->CompanyLogo;?>" target="_blank"><img src="../images/CompanyLogo/<?php echo $row_result->CompanyLogo;?>" width="100" /></a>
     <a href='edit-services?i=<?php echo $_GET['i'];?>&n=<?php echo $row_result->CompanyLogo;?>&c=c' onclick='return DeleteLogo();'>Delete</a>
<?php }?> 
<script>
function DeleteLogo()
{
...
<?php
$query = 'SELECT * FROM `categories` WHERE  `UseFor`="6" ORDER By Title ASC';
$result = $db->query($query);
...
<a href='edit-services?i=<?php echo $_GET['i'];?>&n=<?php echo $row_result->Banner;?>&b=b&Banner=Banner' onclick='return DeleteBanner();'>Delete</a>
<?php }?> 
<script>
...
<img src="../images/Banner/<?php echo $row_result->Banner2;?>" width="100" /></a>
    <a href='edit-services?i=<?php echo $_GET['i'];?>&n=<?php echo $row_result->Banner2;?>&b=b&Banner=Banner2' onclick='return DeleteBanner();'>Delete</a>
    <?php }?> 
    <script>
		function DeleteBanner()
		{
...
<img src="../images/Banner/<?php echo $row_result->Banner3;?>" width="100" /></a>
        <a href='edit-services?i=<?php echo $_GET['i'];?>&n=<?php echo $row_result->Banner3;?>&b=b&Banner=Banner3' onclick='return DeleteBanner();'>Delete</a>
        <?php }?> 
        <script>
            function DeleteBanner()
            {
	  ...
	  <form method="post" action="" name="copyform" id="copyform">	 
         ...
         <!--   <div class="widget-content padded">
				...
				<div id="new_category">
					...
					<option value="">Please select</option>
					 <?php
					$query = "SELECT * FROM `categories` WHERE `Status`='1' AND  `UseFor`='6' ORDER By Title ASC";
					$result = $db->query($query);
					while ($row = $result->fetch_assoc()) 
					{	
					?>

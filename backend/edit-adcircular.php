...
if(isset($_GET['n']) && isset($_GET['c'])) {
 	...
 	$sql  = "UPDATE `tb_ad_circular` SET `CompanyLogo`='' Where `Id_Ad_circular`= '".base64_decode($_GET['i'])."'";
	$res = mysqli_query($db, $sql);
	echo "<script>window.open('edit-adcircular?i=".$_GET['i']."','_self')</script>";
}
...
if(isset($_GET['n']) && isset($_GET['b'])) {
   ...
   $sql  = "UPDATE `tb_ad_circular` SET `".$_GET['Banner']."`=''  Where `Id_Ad_circular`= '".base64_decode($_GET['i'])."'";
		$res = mysqli_query($db, $sql);
	echo "<script>window.open('edit-adcircular?i=".$_GET['i']."','_self')</script>";
}
...
?>
<!DOCTYPE html>
<html><?php require_once('query/edit-adcircular-query.php'); ?>
<head>
<title>Edit AdCircular - <?php echo $row_home->Title;?></title>
...
<script src="../js/ad-circular-validationFun.js"></script>
<script>
      ...
      <div class="row">
        <div class="col-lg-12">
          <div class="widget-container fluid-height clearfix">
            <div class="heading"><i class="icon-edit-sign"></i>Edit AdCircular</div>
...
?>
<?php 
$query = "select * from `tb_ad_circular` where `Id_Ad_circular`= '".base64_decode($_GET['i'])."'";
...
?> 
     ...
     <fieldset>
      <legend><h2>Enter Ad details</h2></legend>
      <div class="form-group">
              <label for="CompanyName">Company  Name:</label>
        <input name="CompanyName" class="form-control" id="CompanyName" type="text" value="<?php echo $row_result->CompanyName;?>" />
    </div>	
...
<select name="Category1" id="Category" class="form-control">
    ...
    {
    ?>
    <option value="<?php echo $row2['Id'];?>" <?php if($row_result->Category1==$row2['Id']){echo 'selected="selected"';}?>><?php echo $row2['Title'];?></option>
    <?php }?>
    </optgroup>
    <?php }?>
</select>
</div>
   <input type="hidden" id="Id" name="Id" value="<?php echo $row_result->Id_Ad_circular;?>" />

    <div class="form-group">
      <label for="Category2">Select another Category (optional):</label>
<select name="Category2"  class="form-control">
    ...
    <div class="form-group">
      ...
      <input name="AdTitle"  id="AdTitle" class="form-control"  type="text" value="<?php echo $row_result->AdTitle;?>" />
    </div>
    
      <div class="form-group">
      <label for="AdURL">Ad URL:</label>
      <input name="AdURL" id="AdURL" class="form-control"  type="text" value="<?php echo $row_result->AdURL;?>" />
     </div>
<div class="form-group">
...
<input name="Telephone" id="Telephone" class="form-control"  type="text" value="<?php echo $row_result->Telephone;?>" />
      ...
      <textarea id="Address" class="widget-content"  name="Address"><?php echo $row_result->Address;?></textarea>
   	 </div>
     
      <div class="form-group">
      <label for="Description">Description:</label>
      <textarea id="Description" class="widget-content" name="Description"><?php echo $row_result->Description;?></textarea>
     </div>
     
      <div class="form-group">
      <label for="Agent">Agent: (optional)</label>
      <input name="Agent" type="text" class="form-control" value="<?php echo $row_result->Agent;?>" />
      </div>
       <div class="form-group">
      <label for="Keywords">Keywords:</label>
      <input name="Keywords" id="Keywords" class="form-control" type="text" value="<?php echo $row_result->CompanyKeywords;?>" />
      </div>
       </fieldset>
     
      
      <fieldset id="menu">
      <legend><h2>Choose your Ad Type</h2></legend>
       <div class="form-group">
      <input type="checkbox" value="2499" id="large"  name="large" class="chk" onclick="showMe('div1')" <?php if($row_result->large=='2499'){echo 'checked="checked"';}?>/>
      <label for="large">Large Top Banner - $2499.00 / month (Pinned at top of the  page)</label>
      </div>
       <div class="form-group">
      <input type="checkbox"  value="1249" id="small" name="small"   class="chk" onclick="showMe('div2')" <?php if($row_result->small=='1249'){echo 'checked="checked"';}?> />
      <label for="stantard">Small Top Banner &nbsp; - $1249.00 / month (Pinned at top of the page)</label>
      </div>
       <div class="form-group">
      <input type="checkbox"  value="499"  id="stantard"  name="stantard"   class="chk" onclick="showMe('div3')" <?php if($row_result->stantard=='499'){echo 'checked="checked"';}?> />
      <label for="small">Standard Ad spot&nbsp; - $499.00 / month</label>
     </div>
      
      <br /><br /><hr /><br />
     <div id="div1" style="display:none" class="upload_div">
           <small>Note: Only JPG, PNG, and GIF formats will be approved. Animated banners are only acceptable for Pinned Ads. Ads uploaded outside of these formats will not be approved.</small>
       <br /><br />
          <p><strong>Upload banner Large Top Spot: </strong><input name="Banner1" type="file" /></p>
...
<a href='edit-adcircular?i=<?php echo $_GET['i'];?>&n=<?php echo $row_result->Banner;?>&b=b&Banner=Banner' onclick='return DeleteBanner();'>Delete</a>
<?php }?> 
<script>
...
<div id="div3" style="display:none" class="upload_div">
           <small>Note: Only JPG, PNG, and GIF formats will be approved. Animated banners are only acceptable for Pinned Ads. Ads uploaded outside of these formats will not be approved.</small>
       <br /><br />
          <p><strong>Upload  Banner for Stantard Ad Spot: </strong><input name="Banner2" type="file" /></p>
          <em> Banner size should be exactly 303 X 174*</em>
...
<a href='edit-adcircular?i=<?php echo $_GET['i'];?>&n=<?php echo $row_result->Banner2;?>&b=b&Banner=Banner2' onclick='return DeleteBanner();'>Delete</a>
<?php }?> 
<script>
function DeleteBanner()
{
return confirm("Are you sure you want to delete this Banner?");
     ...
     <div id="div2" style="display:none" class="upload_div">
        <small>Note: Only JPG, PNG, and GIF formats will be approved. Animated banners are only acceptable for Pinned Ads. Ads uploaded outside of these formats will not be approved.</small>
       <br /><br />
          <p><strong>Upload  Banner for Small Ad spot: </strong><input name="Banner3" type="file" /></p>
          <em> Banner size should be exactly 302 X 165*</em>
...
<a href='edit-adcircular?i=<?php echo $_GET['i'];?>&n=<?php echo $row_result->Banner3;?>&b=b&Banner=Banner3' onclick='return DeleteBanner();'>Delete</a>
<?php }?> 
<script>
function DeleteBanner()
{
